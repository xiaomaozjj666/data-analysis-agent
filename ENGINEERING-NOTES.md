...
- 审计脚本思路：遍历所有"直接含文字"的元素，用祖先背景合成出有效底色，按 4.5:1（小字）/3:1（大字）判定；浅色与暗色、起始页/工作台/产物/历史/命令面板/预览模态各跑一遍。
- 三级文字令牌**按"最差背景"标定**，不是按页面底色：`--fg-subtle` 会在 `--control-hover`(浅) / `--canvas-subtle`(暗) 这类略深的面板底上用于 10~11px 小字，只按页面底算会差一点点（实测 4.2:1）。
