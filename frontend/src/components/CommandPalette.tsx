...
const CommandPalette = React.memo(function CommandPalette({
  ...
  const flat: FlatEntry[] = [
    ...filteredActions.map((a) => ({ type: "action" as const, value: a })),
    ...filteredSessions.map((s) => ({ type: "session" as const, value: s })),
  ];
  ...
  // 按 section 分组动作
  const actionGroups = useMemo<Array<[string, CommandAction[]]>>(() => {
