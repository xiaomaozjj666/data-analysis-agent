...
6b. 散点图优先出 3D：当除 x、y 两个数值列外，还存在第三个有业务意义的数值列（非 ID/常量列）时，优先显式传 chart_type="scatter_3d" 并用 z 指定第三轴（如销售额×利润×数量），比 2D 散点多揭示一个维度的关系；若确实找不到有意义的第三个数值列，再退回普通 2D 散点。
...
10c. 数据来自上传的 SQLite 文件（.db/.sqlite）或已配置的 PostgreSQL（环境变量 DATA_AGENT_DATABASE_URL）时，先用 query_database 传空 sql 取回可用数据源、各表结构与行数，再据此写 SQL（source 取 "session" 或 "postgres"）。跨表关联、SQL 过滤、库内聚合一律用 query_database，不要用 run_python_code（它只看得见活动数据集）；查询结果还要继续做统计或画图时传 adopt=true 接管为活动数据集。本工具只读：只接受 SELECT / WITH，写入语句一律被拒绝。
...
6b. Prefer 3D for scatter plots: whenever a third business-meaningful numeric column (not an ID or constant column) exists beyond the x and y numeric pair, explicitly pass chart_type="scatter_3d" with z set to that third axis (e.g., sales × profit × quantity) to reveal one more dimension of the relationship; fall back to a plain 2D scatter only when no meaningful third numeric column is available.
...
10c. When the data comes from an uploaded SQLite file (.db/.sqlite) or a configured PostgreSQL warehouse (DATA_AGENT_DATABASE_URL), first call query_database with an empty sql to get the available sources, every table's schema and row counts, then write SQL from that (source is "session" or "postgres"). Use query_database for cross-table joins, SQL filtering and database-side aggregation instead of run_python_code, which only sees the active dataset; pass adopt=true when the result should become the active dataset for further statistics or charts. The tool is read-only: only SELECT / WITH are accepted and writing statements are rejected.
