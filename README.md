...
- **大数据图表的可读性**：几十万行时"每点一个标记"的散点会因 alpha 饱和糊成一团（实测 30 万点全是同色噪点）。**散点 ≥ 2 万行自动切换为密度视图**：服务端聚合成网格（单格约 7px）、用分档颜色表达每格记录数（档位图例直接给出"1 / 2–4 / 5–9 …"的记录数区间）、零记录格完全透明，并在有分组时按分组拆成共享色标的分面面板（类别用分面表达、颜色只表达密度）；无分组时补顶部/右侧边缘直方图（jointplot 布局）、趋势线、均值参考线与最外围原始点。判定"是否真有聚集"会先扣掉均匀随机点云的统计涨落，没有聚集就直说。直方图 ≥ 5 万行改为服务端分箱（纵轴仍是真实记录数）、箱线图改为服务端五数概括、小提琴图分层抽样——**同一批图表 HTML 体积从 3~12MB 降到 20~200KB**，密度视图是精确聚合而非抽样，完整数据始终保留在同名 JSON 产物里。
